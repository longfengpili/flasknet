...
def show():
    ...
    if ip in cf.iplist and browser_name is not None:
        setting = db.session.query(alarm_setting).filter(alarm_setting.data_ts >= today).order_by(alarm_setting.app_name).order_by(alarm_setting.platform).order_by(db.desc(alarm_setting.total_times)).order_by(db.desc(alarm_setting.current)).order_by(db.desc(alarm_setting.last_mail_time))
